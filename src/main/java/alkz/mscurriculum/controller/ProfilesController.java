...
public class ProfilesController {
  ...
  public ResponseEntity<ResultDto<ProfileDto.Response>> getProfileById(String id) {
    return ResponseEntity.ok(new ResultDto<>(service.findById(id)));
  }

  @GetMapping(V1_PATH + "/users/{userId}")
  public ResponseEntity<ResultDto<ProfileDto.Response>> getProfileByUserId(@PathVariable String userId) {
    return ResponseEntity.ok(new ResultDto<>(service.findByIdUser(userId)));
  }

  @PostMapping(V1_PATH)
  public ResponseEntity<ResultDto<ProfileDto.Response>> createProfile(ProfileDto.Request request) {
